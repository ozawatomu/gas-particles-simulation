...
public class ParticleSimulation2D : MonoBehaviour
{
    ...
    Particle[] SpawnParticles()
    {
        ...
        float targetVelocityMagnitude = Mathf.Sqrt(2 * temperature);
        for (int i = 0; i < particleCount; i++)
        {
            ...
            particles[i] = new Particle
            {
                ...
                velocity = particleDirection * targetVelocityMagnitude,
            };
        }
    ...
    {
        Particle[] particlesCPU = new Particle[particleCount];
        particleReadBuffer.GetData(particlesCPU);
        float totalKineticEnergy = 0.0f;
        for (int particleI = 0; particleI < particleCount; particleI++)
        {
            float velocitySquared = particlesCPU[particleI].velocity.sqrMagnitude;
            totalKineticEnergy += 0.5f * velocitySquared; // mass = 1
        }
        float averageKineticEnergy = totalKineticEnergy / particleCount;
        Debug.Log($"Average Kinetic Energy: {averageKineticEnergy:F4}");
        float velocityScale = Mathf.Sqrt(temperature / averageKineticEnergy);
        for (int particleI = 0; particleI < particleCount; particleI++)
        {
            particlesCPU[particleI].velocity *= Mathf.Lerp(1, velocityScale, Time.deltaTime);
        }
        particleReadBuffer.SetData(particlesCPU);
    }
