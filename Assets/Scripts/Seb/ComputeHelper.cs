...
namespace Seb.Helpers
{
    ...
    public static class ComputeHelper
    {
        ...
        public static int CalculateThreadGroupCount1D(
            ComputeShader cs,
            int numIterationsX,
            int kernelIndex = 0
        )
        ...
        public static ComputeBuffer CreateStructuredBuffer<T>(List<T> data)
            where T : struct
        {
        ...
        public static void CreateStructuredBuffer<T>(ref ComputeBuffer buffer, List<T> data)
            where T : struct
        {
            int stride = GetStride<T>();
            bool createNewBuffer =
        ...
        public static void SetBuffers(
            ComputeShader cs,
            int kernel,
            Dictionary<ComputeBuffer, string> nameLookup,
            params ComputeBuffer[] buffers
        )
        {
        ...
        public static ComputeBuffer CreateAndSetBuffer<T>(
            T[] data,
            ComputeShader cs,
            string nameID,
            int kernelIndex = 0
        ...
        public static void CreateAndSetBuffer<T>(
        ...
        public static void CreateRenderTexture3D(
            ref RenderTexture texture,
            int size,
            GraphicsFormat format,
        ...
        public static void CreateArgsBuffer(ref ComputeBuffer buffer, ComputeBuffer appendBuffer)
        {
            ...
            if (
                buffer == null
                || buffer.stride != stride
                || buffer.count != singleInstanceRenderArgs.Length
                || !buffer.IsValid()
            )
            {
                buffer = new ComputeBuffer(
                    singleInstanceRenderArgs.Length,
                    stride,
                    ComputeBufferType.IndirectArguments
                );
            }
        ...
        public static void CreateArgsBuffer(
            ref ComputeBuffer argsBuffer,
            Mesh mesh,
            int numInstances
        )
        {
            ...
            bool createNewBuffer =
                argsBuffer == null
                || !argsBuffer.IsValid()
                || argsBuffer.count != argsBufferArray.Length
        ...
        )
        {
            ...
            foreach (var field in fields)
            {
                ...
                if (
                    fieldType == typeof(UnityEngine.Vector4)
